...
let TenantGuard = class TenantGuard {
    canActivate(context) {
        ...
        if (user.rol === role_enum_1.Role.SUPER_ADMIN) {
            return true;
        }
        if (requestedClubId && user.clubId && requestedClubId !== user.clubId) {
            throw new common_1.ForbiddenException('Acceso multi-tenant denegado: No perteneces a la academia o club solicitado');
        }
        return true;
    }
};
