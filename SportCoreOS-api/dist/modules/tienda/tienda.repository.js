...
let TiendaRepository = class TiendaRepository {
    ...
    async findCatalogoByClub(clubId, options) {
    ...
    async findPedidosByClub(clubId, options) {
        const isPaginated = options?.page !== undefined || options?.limit !== undefined;
        const page = Math.max(1, Number(options?.page) || 1);
        const limit = Math.min(100, Math.max(1, Number(options?.limit) || (isPaginated ? 10 : 50)));
        const offset = (page - 1) * limit;
       ...
       WHERE ${whereParts.join(' AND ')}`, params);
        const total = parseInt(countRes.rows[0]?.count || '0', 10);
        const queryParams = [...params, limit, offset];
        const dataRes = await this.db.query(`SELECT ped.*, p.nombre as producto_nombre, p.codigo_sku, v.talla,
