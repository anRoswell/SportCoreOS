...
let BiometriaService = class BiometriaService {
    ...
    async registrarEvaluacion(clubId, evaluadorId, data) {
        const m = data.tallaCm ? data.tallaCm / 100 : null;
        const imc = m && data.pesoKg ? parseFloat((data.pesoKg / (m * m)).toFixed(1)) : null;
        const res = await this.db.query(`INSERT INTO rendimiento.evaluaciones_biometricas (
    ...
    async getHistorialJugador(jugadorId) {
        const res = await this.db.query(`SELECT eb.*, CONCAT(u.nombre, ' ', u.apellido) as evaluador_nombre
