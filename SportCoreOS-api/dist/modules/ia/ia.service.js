...
let IaService = class IaService {
    ...
    async generarBoletinAlumno(clubId, dto) {
        const data = await this.iaRepo.getHistorialJugadorParaBoletin(dto.jugador_id, clubId);
        if (!data.jugador) {
            throw new common_1.NotFoundException('Jugador no encontrado en este club');
        }
        const { jugador, biometria, partidos } = data;
        const bioActual = biometria[0] || null;
        const minutosTotales = partidos.reduce((acc, p) => acc + (p.minutos_jugados || 0), 0);
        const golesTotales = partidos.reduce((acc, p) => acc + (p.goles || 0), 0);
        const asistenciasTotales = partidos.reduce((acc, p) => acc + (p.asistencias || 0), 0);
        const fortalezas = [];
        const mejoras = [];
        ...
        if (fortalezas.length === 0)
            fortalezas.push('Buena disciplina táctica y disposición al aprendizaje.');
        if (mejoras.length === 0)
            mejoras.push('Mantener el plan de fortalecimiento muscular y descanso preventivo.');
...
El acompañamiento de los padres es vital en esta etapa formativa. Felicitamos la puntualidad, actitud deportiva y respeto hacia compañeros y rivales.
    `.trim();
        const log = await this.iaRepo.guardarLogGeneracion(clubId, jugador.id, 'BOLETIN_PADRES_MENSUAL', 350, 420, contenidoGenerado, { periodo: dto.periodo || new Date().toISOString().slice(0, 7), enfoque: dto.enfoque_adicional });
        return {
    ...
    async chatTacticoDt(clubId, dto) {
        let contextoPlantel = '';
        if (dto.categoria_id) {
...
exports.IaService = IaService = __decorate([
    ...
    __metadata("design:paramtypes", [ia_repository_1.IaRepository])
], IaService);
