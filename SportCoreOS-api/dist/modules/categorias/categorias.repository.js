...
let CategoriasRepository = class CategoriasRepository extends base_repository_1.BaseRepository {
    constructor(db) {
        super(db, 'deportivo.categorias');
    }
    async findCategoriasByClub(clubId) {
        const res = await this.db.query(`SELECT c.id, c.nombre, c.codigo_categoria, c.anio_nacimiento_min, c.anio_nacimiento_max,
       ...
       ORDER BY c.anio_nacimiento_max ASC`, [clubId]);
        return res.rows;
    }
    async findPlantelByCategoria(categoriaId, clubId) {
        const res = await this.db.query(`SELECT j.id, j.nombres, j.apellidos, j.numero_documento, j.fecha_nacimiento,
       ...
       ORDER BY j.numero_dorsal ASC NULLS LAST, j.apellidos ASC`, [categoriaId, clubId]);
        return res.rows;
    }
    async createCategoria(clubId, data) {
        const res = await this.db.query(`INSERT INTO deportivo.categorias (
    ...
    async updateCategoria(id, clubId, data) {
        const fields = [];
    ...
    async deleteCategoria(id, clubId) {
        const res = await this.db.query(`UPDATE deportivo.categorias SET activa = false, updated_at = NOW() WHERE id = $1 AND club_id = $2 RETURNING *`, [id, clubId]);
        return res.rows[0] || null;
    }
};
